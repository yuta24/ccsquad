...
export class DagOrchestrator {
  ...
  private buildPrompt(jobId: string): string {
    return [
      ...
      `2. phase_config.type と phase_config.agent を確認`,
      `3. type が review の場合は停止して報告`,
      `4. それ以外は .claude/agents/{agent}.md のエージェントとして作業を実行`,
      `5. 完了後 ccsquad job transition ${jobId} completed --message "要約"`,
      `6. 失敗時 ccsquad job transition ${jobId} failed --message "理由"`,
